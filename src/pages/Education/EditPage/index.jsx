...
import {FLAG_ADD, FLAG_RELEASE, FLAG_SEE, FLAG_SUBMIT, handleEmptyStr} from "@/utils/utils";
...
class EditPage extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState({
      ...
      this.toUpdateClassIdOfModel() // 更新model上的存储的classId
      ...
      if(this.props.type === FLAG_RELEASE){
      ...
      payload:{
        ...
        cb:(res)=>{
          this.setState({ detialInfo: res },() => {
            const { detialInfo } = this.state
            this.coverChild.setCoverIamgeOfModel(detialInfo.imagebUrl)
            const { showType } = this.state
            if(showType !== FLAG_ADD ){ //
              this.setState({
                isCoverComplete: detialInfo.imagebUrl? true: false,
                visiableCoverImage: detialInfo.imagebUrl , // 封面模块是否课件
                // visiableAudit: showType === FLAG_SUBMIT, // 审核
              })
            }
          })
        }
  ...
  render() {
    const { resourceType } = this.props.eduManageModule;
    const { showType, visiableBaseInfo , visiableCoverImage, visiableUpLode, visiableAudit , id, detialInfo } = this.state;
