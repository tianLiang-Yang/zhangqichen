...
@connect(({ doctorQualificationReview }) => ({
...
class DoctorHistory extends React.Component {
  ...
  getTableList = (page, size) => {
    ...
    dispatch({
      type: 'doctorQualificationReview/fetchDoctorHistory',
      payload: {
        userId,
        page,
        size,
  ...
  render() {
    const { doctorAduitHistroy, isLoadding } = this.props.doctorQualificationReview;
    const statusData = [
